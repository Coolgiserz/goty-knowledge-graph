...
- **🌐 交互式数据探索（新增）**：内置 **FastAPI 探索 API + 原生 ES Module 探索 SPA**。页面**默认直接呈现标准口径下的静态分析结果**（无需操作）；用户点「自定义探索」才进入参数调节，结果在独立面板展示、**不覆盖默认分析**。后端仍区分「数据有效性 × 解读有效性」，但前端不以前端告警方式提示「解读失效」——用户自行探索只是缺少预设解读，无需显式提示。
