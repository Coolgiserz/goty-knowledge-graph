...
// vis-network
const groupColors = {
  goty:{color:'#f5b301',border:'#ffd54a'},
  game:{color:'#3b6ea5',border:'#5b8fd0'},
  studio:{color:'#8e44ad',border:'#b06fd0'},
...
function renderDetail(id){
  ...
  if(n.group==='game' || n.group==='goty'){
    ...
    html += `</dl>`;
    if(r.developer_id) html += `<div style="margin-top:12px"><span class="gamelink" onclick="selectNode('${r.developer_id}')">查看开发商：${esc(r.developer)} 的全部作品 →</span></div>`;
    el.innerHTML = html;
...
net.on('click',p=>{
  ...
  else {clearHighlight();net.unselectAll();renderDetail(null);}
});
document.getElementById('detailClose').onclick=()=>{clearHighlight();net.unselectAll();renderDetail(null);};
document.addEventListener('keydown',e=>{if(e.key==='Escape'){clearHighlight();net.unselectAll();renderDetail(null);}});
...
function applyFilters(){
  ...
  edgesDS.update(GRAPH.edges.map(e=>({id:e.id, hidden:!(visNodes.has(e.from)&&visNodes.has(e.to))})));
}
...
document.getElementById('reset').onclick=()=>{
  ...
  clearHighlight();net.unselectAll();renderDetail(null);
  applyFilters();net.fit({animation:true});
};

// ---------- table view ----------
let currentView='graph';
function buildTable(){
  const rows=GRAPH.nodes.filter(n=>n.group==='game'||n.group==='goty')
