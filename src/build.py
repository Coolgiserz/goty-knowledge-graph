...
# 分类模型分两层：
#   (a) 玩法类别（游戏“玩什么”）：12 个顶层原子类别 + 子类型，构成 SUBCLASS_OF 层级
#   (b) 设计维度（跨玩法的“特征标签”，可与任意玩法类别叠加）：
#         开放世界 / 多人合作 / 在线 / 虚拟现实
# 这样「开放世界」是一等公民——艾尔登法环、旷野之息、天际、GTA、巫师3 等
# 都能被正确归类与筛选，而不是被塞进一个“开放世界动作角色扮演”的复合叶子。
GENRE_PARENT = {
    # 顶层：玩法类别（12）
    "角色扮演": None, "动作": None, "射击": None, "冒险": None,
    "动作冒险": None, "平台跳跃": None, "策略": None, "模拟": None,
    "竞速": None, "卡牌": None, "解谜": None, "虚拟现实": None,
    # 顶层：设计维度（跨玩法，可与玩法类别叠加；自身为顶层、无父/子）
    "开放世界": None, "多人合作": None, "在线": None,
...
# 原始 genre 字符串 -> 原子“玩法”子类型列表（只含玩法，不含设计维度；
# 设计维度在下方依据 原始串 / 描述文本 / 策划标题集 统一补标）。
RAW_TO_LEAVES = {
    ...
    "卡牌角色扮演": ["数字卡牌", "角色扮演"],           # 拆分：卡牌 + 角色扮演
    "即时战略（RTS）": ["即时战略"],
    "合作冒险": ["合作冒险"], "合作动作冒险": ["动作冒险"],  # “合作”走设计维度
    "回合制角色扮演（含即时元素）": ["回合制角色扮演"],
    "在线动作角色扮演": ["动作角色扮演"],                # “在线”走设计维度
    "增强现实 / 硬件展示": ["增强现实"],
    "多人在线角色扮演": ["大型多人在线角色扮演"],          # “在线”走设计维度
    "大型多人在线角色扮演（MMORPG）": ["大型多人在线角色扮演"],
    "太空角色扮演": ["太空角色扮演"], "平台动作": ["平台动作"],
    "平台跳跃": ["平台跳跃"],
    "开放世界动作冒险": ["动作冒险"],                     # 开放世界 -> 设计维度
    "开放世界动作角色扮演": ["动作角色扮演"],             # 开放世界 -> 设计维度
    "开放世界西部动作冒险": ["西部动作冒险"],              # 开放世界 -> 设计维度
    "开放世界角色扮演": ["动作角色扮演"],                # 开放世界 -> 设计维度（本数据集皆为动作RPG）
    "开放世界角色扮演（资料片）": ["动作角色扮演"],
    ...
    "第一人称射击": ["第一人称射击"],
    "第一人称射击RPG": ["第一人称射击", "动作角色扮演"],   # 拆分：射击 + 动作RPG
...
# ---------- 设计维度：跨玩法标签（开放世界 / 多人合作 / 在线）----------
# 依据三重来源，确保逻辑正确、不漏标：
#   1) 原始 genre 字符串中的关键词（开放世界 / 合作 / 在线）
#   2) 描述 / 玩法 / 简介 文本中的“开放世界”关键词
#   3) 策划标题集（覆盖“描述未写但公认如此”的游戏，如 GTA 全系、辐射3/4、RDR2、暗黑4 等）
OPEN_WORLD_TITLES = {
    "上古卷轴IV：湮没", "上古卷轴V：天际",
    "侠盗猎车手IV", "侠盗猎车手V", "侠盗猎车手III",
    "侠盗猎车手：罪恶都市", "侠盗猎车手：圣安地列斯",
    "荒野大镖客：救赎", "荒野大镖客：救赎2",
    "辐射3", "辐射4", "星空", "辐射76",
    "龙腾世纪：审判", "巫师3：狂猎", "赛博朋克2077", "赛博朋克2077：往日之影",
    "神界2：龙刃", "暗黑破坏神IV",
    "艾尔登法环", "塞尔达传说：旷野之息", "塞尔达传说：王国之泪",
}
COOP_TITLES = {
    "双人成行", "逃出生天", "双影奇境", "兄弟：双子传说",
    "神界：原罪2", "博德之门3", "守望先锋", "守望先锋2",
}
ONLINE_TITLES = {
    "守望先锋", "守望先锋2", "魔兽世界", "辐射76", "圣歌", "喷射战士2",
}

def design_dims(rec):
    """返回某游戏的设计维度集合（开放世界 / 多人合作 / 在线）。"""
    raw = rec.get("genre") or ""
    zh = rec.get("title_zh") or ""
    txt = (rec.get("gameplay", "") + rec.get("unique_features", "")
           + rec.get("description", ""))
    dims = set()
    if ("开放世界" in raw) or ("开放世界" in txt) or (zh in OPEN_WORLD_TITLES):
        dims.add("开放世界")
    if ("合作" in raw) or (zh in COOP_TITLES):
        dims.add("多人合作")
    if ("在线" in raw) or (zh in ONLINE_TITLES):
        dims.add("在线")
    return dims
...
for gid, rec in games.items():
    ...
    # 原子“玩法”叶子（来自原始 genre 字符串）
    leaves = list(RAW_TO_LEAVES.get(raw, [raw] if raw else []))
    # 叠加“设计维度”（开放世界 / 多人合作 / 在线）
    leaves += sorted(design_dims(rec))
    # 开放世界的“动作”类游戏同时归入「动作冒险」：
    # 这样“开放世界 ∩ 动作冒险”的交集才逻辑自洽（艾尔登法环、天际、巫师3、
    # 赛博朋克2077、辐射3/4、星空、龙腾世纪：审判 等开放世界动作RPG 本就兼具探索冒险）。
    if "开放世界" in leaves and ({"动作角色扮演", "西部动作冒险", "动作冒险", "平台动作"} & set(leaves)):
        if "动作冒险" not in leaves:
            leaves.append("动作冒险")
    # 兜底：未知玩法串也登记为顶层类型
    for lv in leaves:
        if lv not in genre_id:
            genre_id[lv] = f"genre_{len(genre_id)+1:03d}"
            GENRE_PARENT.setdefault(lv, None)
    rel_genre.extend((gid, genre_id[lv]) for lv in leaves)
    # 顶层类别：玩法叶子取各自顶层；设计维度自身即顶层
    tops = set()
    for lv in leaves:
        if lv in GENRE_PARENT and GENRE_PARENT[lv] is None:
            tops.add(lv)
        else:
            t = genre_top(lv)
            if t:
                tops.add(t)
    rec["genres"] = leaves                     # 原子类型列表（含设计维度，展示用）
    rec["tiers"] = sorted(tops)                # 顶层类别列表（筛选用）
    if rec["is_goty"]:
        aid = f"award_{gid}"
...
- **{GRAPH['stats']['genres']}** 个游戏类型（含 **{GRAPH['stats']['top_genres']}** 个顶层原子类别：12 个玩法类别 + 3 个设计维度「开放世界 / 多人合作 / 在线」，构成层级体系；已合并同义词、拆分复合词、把“开放世界”等从修饰词提升为一等类别）
...
- `(:Genre)-[:SUBCLASS_OF]->(:Genre)` — 类型层级（子类型隶属于父类型，直至 12 个玩法顶层类别；设计维度「开放世界 / 多人合作 / 在线」为独立顶层，无父/子）
